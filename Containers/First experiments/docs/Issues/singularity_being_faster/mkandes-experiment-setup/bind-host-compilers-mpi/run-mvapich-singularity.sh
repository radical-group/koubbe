...
time -p singularity exec --cleanenv --bind /etc,/opt,/usr,/oasis,/scratch centos-repo.simg mpicc mpi_hello_world.c -o hello_world_intel_mvapich2_ib.x
time -p ibrun singularity exec --bind /etc,/opt,/usr,/oasis,/scratch centos-repo.simg ./hello_world_intel_mvapich2_ib.x
