...
def GeneratePipeline(pcfg, ecfg, pipe_name, exp_dir):
	...
	for this_stage in stage_names:
		if this_stage in pcfg.keys():
			...
			p.add_stages(GenerateStage(pcfg[this_stage], ecfg, p.name, this_stage, exp_dir))
	
	return(p)


def GenerateStage(scfg, ecfg, pipe_name, stage_name, exp_dir):
	...
	for this_task in scfg.keys():
		...
		s.add_tasks(GenerateTask(scfg[this_task], ecfg, pipe_name, stage_name, this_task, exp_dir))
	
	# Return the stage object
	return(s)


def GenerateTask(tcfg, ecfg, pipe_name, stage_name, task_name, exp_dir):
	...
	if rcfg['resource-desc']['name'] != 'local.localhost':
		t.pre_exec += [tcfg['pre_exec2']]
	...
	t.arguments = [tcfg['executable_args']] + [tcfg['container']] + [tcfg['container_args']] + [tcfg['script']] + match_options(tcfg['options'], ecfg['options'])
