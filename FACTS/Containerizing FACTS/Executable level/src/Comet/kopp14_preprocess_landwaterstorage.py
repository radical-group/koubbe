# Temporary workaround so the python packages get recognized when RP launches the task
import sys
...
''' kopp14_preprocess_landwaterstorage.py
Code generated 19-08-2019, by Tim Hermans
Last edited 21-08-2019, by Gregory Garner
This script runs the land water storage pre-processing task for the Kopp 2014 workflow. 
...
def kopp14_preprocess_landwaterstorage(dotriangular, includepokhrel, pipeline_id):
	...
	yrs = np.linspace(2010,2200,20) # target years projections
	...
	popscenfile		= 'UNWPP2012 population projections.csv'	
	...
	with open(popscenfile,'r', newline='') as csvfile:
		...
		popscen	  = np.zeros([row_count-1,3])
	
		for row in popdata:
			popscenyr[i] = row[0] # store years
			popscen[i,:] = row[1:4] # store population projections
			i += 1
	...
	output = {'dgwd_dt_dpop_pcterr': dgwd_dt_dpop_pcterr, 'dam_pcterr': dam_pcterr,\
				'yrs': yrs, 'dotriangular': dotriangular, 'includepokhrel': includepokhrel,\
				'pop0': pop0, 't0':t0}
...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description="Run the land water storage pre-processing stage for the Kopp14 SLR projection workflow",\
